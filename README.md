...
**운영 모드 2가지**
- **시뮬레이션**: 실제 주문 없이 가상 예산으로 매매. **가상 예산은 직접 입력**하거나(대시보드 입력칸 / 텔레그램 `/start_sim 500000`), 비워두면 **현재 업비트 KRW 잔고**를 사용
...
### 타임프레임별 점수 (`analysis.py` → `score_signal()`) — **추세 정렬형 (2026-06-13 재설계)**

> **재설계 배경**: 구버전은 RSI 과매도·BB 하단 이탈에 큰 가점을 주는 순수 평균회귀(역추세)
> 모델이었다. 일봉 가중치가 2.5배라 '강한 매수' = '일봉 깊은 과매도 = 하락추세 한복판
> 진입(떨어지는 칼 잡기)'을 의미했고, 백테스트에서 거래당 기대값 **-0.56%**(승률 47.7%)로
> 일관된 음의 우위가 확인됐다. 신버전은 **추세 레짐을 1차 게이트**로 삼고, 상승추세의
> 눌림목·모멘텀 전환에 가점하며 비추세 과매도 매수를 차단한다.

각 지표가 점수에 기여하는 방식 (es/em/el = EMA short/mid/long):

| 블록 | 조건 | 점수 |
|------|------|------|
| **추세 레짐** | 정배열 (es>em>el) | +2 |
| | 역배열 (es<em<el) | -2 |
| **MACD 모멘텀** | 골든크로스 (히스토그램 음→양) | +3 |
| | 데드크로스 (히스토그램 양→음) | -3 |
| | 상승 모멘텀 강화 (양수 & 증가) | +2 |
| | 하락 모멘텀 강화 (음수 & 감소) | -2 |
| **RSI (상승추세 시)** | 40~60 추세 내 눌림목 | +2 |
| | < 40 깊은 눌림(추세가 받침) | +1 |
| | > 78 과열 | -1 |
| **RSI (비추세 시)** | > 70 / > 60 | -2 / -1 |
| **볼린저 (상승추세 시)** | bb_pct ≥ 0.8 상단 주행(강세 지속) | +1 |
| | bb_pct ≤ 0 추세 이탈 경고 | -1 |
| **볼린저 (비추세 시)** | bb_pct ≥ 1.0 / ≥ 0.8 | -2 / -1 |
| **스토캐스틱** | 상승추세 & K < 40 단기 눌림 | +1 |
| | K > 85 과열 | -1 |
| **거래량** | ≥ 2.0배 & 점수 양수 / 음수 | +1 / -1 |

> 핵심: **비추세(하락·혼조) 과매도에는 가점하지 않아 knife-catching을 차단**한다.
...
|-------------|-------------|--------------|-------------|
| ≥ 8 | 강한 매수 | buy-strong | 후보 (단, 아래 임계치 충족 시만 실행) |
...
> **실제 신규 매수 임계치 = `BUY_SCORE_THRESHOLD`(기본 12)**. 재설계 룰의 양의 기대값
> 구간이 12 이상이라 라벨 임계치(8)보다 높게 잡는다. 추가로 **시장 레짐 필터**
> (`MARKET_REGIME_FILTER`, 기본 ON)가 BTC 일봉 하락 추세(단기<중기 EMA) 구간의 신규
> 매수를 전 종목 차단한다. → **재설계 점수 + 임계치 12 + 레짐 필터**가 한 세트로
> 양의 기대값을 만든다(아래 백테스트 참조).
...
② run_auto_trade() — 활성화된 경우만 실행
   ...
   Phase 2: 신규 매수 — 차단 게이트 통과 시에만
            (일일 손실 한도 · 연속 손절 쿨다운 · **시장 레짐(BTC 하락)** ·
             F&G 양극단 · 시장 캐시 신선도) · 점수 ≥ BUY_SCORE_THRESHOLD(12)
        ↓
...
| `/start_sim [금액]` | 시뮬레이션 시작. 금액 지정 시 그 값을 가상 KRW 예산으로 사용(`/start_sim 500000`). 금액 생략 시 입력을 유도(다음 메시지로 숫자 전송), `잔고` 입력 시 업비트 잔고 사용 |
...
> **백테스트 발견 (요주의):** 최근 90일·10종목 구간에서 구 진입 룰(score≥8)은 **거래당 기대값이 음수**(약 -0.6%)였다. 익절(+5%)은 드물게 발동하고 손절·매도신호가 지배해 보상:위험이 비대칭이었다. 디바운스는 개선 효과가 없었다. → **아래 진입 신호 재설계로 이어짐.**

### 2026-06-13 진입 신호 재설계 (역추세 → 추세 정렬 + 시장 레짐 필터)

위 음의 기대값 진단을 근거로 `score_signal`을 **추세 정렬형으로 재설계**하고, **시장 레짐 필터**를 도입했다. 모든 변경은 `scripts/backtest.py`로 검증.

| 분류 | 항목 | 내용 |
|------|------|------|
| 진입 로직 | `score_signal` 재설계 | 평균회귀(과매도·BB하단 매수) → **추세 레짐 1차 게이트 + 모멘텀 확인**. 비추세 과매도 매수(knife-catching) 차단. (§6 점수표 참조) |
| 게이트 | 시장 레짐 필터 | `MARKET_REGIME_FILTER`(기본 ON) — BTC 일봉 단기<중기 EMA(하락 추세)면 **전 종목 신규 매수 차단**. 10분 캐시. 하락장 롱 진입 차단이 양의 기대값의 핵심 |
| 파라미터 | 진입 임계치 | `BUY_SCORE_THRESHOLD`(기본 12) — 하드코딩 `≥8`을 설정값화. 재설계 룰의 양의 기대값 구간이 12+ |
| 검증 인프라 | 백테스터 확장 | `--regime-gate`(레짐 필터), `--no-signal-exit`(신호청산 토글), **Buy&Hold 벤치마크** 출력 추가. 기본 임계치 12로 라이브와 일치 |

> **백테스트 검증 (365일·10종목, Buy&Hold -39%인 하락장):**
>
> | 구성 | 거래당 기대값 | 승률 | N |
> |------|------|------|---|
> | 구 룰 (역추세, 게이트 없음) | **-0.412%** | 50.4% | 1733 |
> | 구 룰 + 레짐게이트 | -0.232% | 55.0% | 555 |
> | 재설계 (게이트 없음) | -0.147% | 39.4% | 1306 |
> | **재설계 + 레짐게이트 (th=12)** | **+0.149%** | 48.2% | 635 |
> | **재설계 + 레짐게이트 (th=14)** | **+0.286%** | 49.3% | 517 |
>
> **재설계 점수 + 레짐 필터 + 임계치 12가 한 세트**일 때만 양의 기대값. 시장이 39% 빠지는
> 동안 양의 EV를 낸 것이 핵심. 단, **최근 90일 급락 구간**은 BTC가 알트보다 버텨 게이트가
> 적게 작동, 여전히 음수(-0.39%)지만 종목별 MDD가 구 룰 -95.7% → -3~-20%로 급감. 단일
> 표본·종가 청산·1분봉 생략 한계는 유효하므로 실거래 전 시뮬 병행 권고.

#### 백테스터 사용법
```bash
# 라이브 동등 설정(기본 th=12) + 시장 레짐 필터, 365일
python scripts/backtest.py --days 365 --regime-gate
# 구 90일 비교 / 디바운스 효과
