...
class KisClient:
    ...
    def get_us_ohlcv(self, symbol: str, excd: str, count: int = 100) -> "pd.DataFrame":
        url = f"{self._auth.base_url}/uapi/overseas-price/v1/quotations/dailyprice"
        params = {
            "AUTH": "",
            "EXCD": excd,
            "SYMB": symbol,
        ...
        try:
            ...
            for r in rows:
                ...
                if not xymd:
                    continue
                try:
                    dt = datetime.strptime(xymd, "%Y%m%d").replace(tzinfo=_KST)
                except ValueError:
                    continue
                records.append({
                    "datetime": dt,
            ...
            import logging
            logging.getLogger(__name__).warning("KIS 해외주식 OHLCV 조회 실패 (%s %s): %s", excd, symbol, e)
            return pd.DataFrame()

    # ── 해외주식 현재가 ──────────────────────────────────────────────────────

    def get_us_current_price(self, symbol: str, excd: str) -> "float | None":
        ...
        try:
            resp = requests.get(url, headers=self._headers("HHDFS00000300"),
                                params=params, timeout=10)
            resp.raise_for_status()
            data = resp.json()
            self._check_rt(data)
            output = data.get("output") or {}
            val = float(output.get("last", 0) or 0)
            return val if val > 0 else None
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning("KIS 해외주식 현재가 조회 실패 (%s %s): %s", excd, symbol, e)
            return None

    # ── 해외주식 소수점 주문 ─────────────────────────────────────────────────

    def place_us_order_fractional(
        self, symbol: str, excd: str, side: str, amount_usd: float, current_price: float
    ) -> bool:
        import logging
        tr_id = "TTTT1007U" if side == "buy" else "TTTT1008U"
        url = f"{self._auth.base_url}/uapi/overseas-stock/v1/trading/order"
        qty = round(amount_usd / current_price, 4)
        qty_str = str(qty)
        body = {
            "CANO":          config.KIS_ACCOUNT_NO_REAL,
            "ACNT_PRDT_CD":  "01",
            "OVRS_EXCG_CD":  excd,
            "PDNO":          symbol,
            "ORD_DVSN":      "00",
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                "KIS 해외주식 주문 실패 (%s %s %s): %s", side, excd, symbol, data.get("msg1", "")
            )
            return False
        except Exception as e:
            logging.getLogger(__name__).warning("KIS 해외주식 주문 오류 (%s %s %s): %s", side, excd, symbol, e)
            return False

    def get_usd_krw_rate(self) -> float:
        """USD/KRW 환율 조회. open.er-api.com 사용, 1시간 캐시.
        성공 시 내부 변수(_fx_last_rate) 갱신.
        실패 시 마지막 성공 환율 유지 — 첫 조회도 실패한 경우에만 config.US_DEFAULT_EXCHANGE_RATE 사용."""
        import time as _time
        now = _time.time()
        last_rate: float = getattr(self, "_fx_last_rate", 0.0)
        last_ts: float = getattr(self, "_fx_last_ts", 0.0)
        if now - last_ts < 3600 and last_rate > 0:
            return last_rate
        try:
            resp = requests.get(
            ...
            self._fx_last_rate = rate
            self._fx_last_ts = now
            return rate
        except Exception:
            if last_rate > 0:
                return last_rate
